先讲下react16的生命周期
 # 挂载阶段
1. constructor
2. getDefaultProps：MyComponent.defaultProps = { age: 'unknown' }
3. getInitialState：
是因为这两个函数只是在ES2015语法中创建组件时暴露出来，在ES6语法中我们通过两个赋值语句实现了同样的效果。

4. componentWillMount() : void 17中弃用，需加上unsafe
...
组件渲染完成，真实的DOM被渲染出来后调用
 # 更新阶段
1. componentWillReceiveProps(nextProps) : void
17中弃用，需加上unsafe
